...
export const logOut = async (req,res) => {
    try {
        res.clearCookie("token",{httpOnly: true, secure: true, sameSite: 'none'}).send({message: 'logout success fully!',success: true})
    } catch (error) {
        res.status(500).send({error: error.message})
    }
}
...
export const loginUser = async (req,res) => {
    try {
        ...
        if (userDetails &&(await bcrypt.compare(password, userDetails.password))) {
            ...
            res.cookie('token', token,{httpOnly: true, secure: true, sameSite: 'none'}).send({message: 'login success!',success: true})
        }else{
